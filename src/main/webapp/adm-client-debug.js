...
Ext.define('AdmClient.view.Main', {
	...
	initComponent : function() {
		this.items = [ {
			...
			items : [ {
				title : 'Welcome',
				html : '<center>An appealing welcome page - with tutorials etc? </center>'
			},{
				title : 'Settings',
				items : [ {
					xtype : 'settings'
...
Ext.define('OpenEMap.GeoExt.tree.View', {});
 ...
 * 
 */
Ext.define('OpenEMap.view.Layers' ,{
...
Ext.define('OpenEMap.Gui', {
    ...
    createPanels: function(items) {
        ...
        if (this.gui.layers instanceof Ext.Container) {
            ...
            this.mapLayers = Ext.create('OpenEMap.view.Layers', Ext.apply({
                mapPanel : this.mapPanel
            }, this.gui.layers));
        }
...
Ext.define('AdmClient.controller.MapConfiguration', {
	...
	panMap : function(btn) {
		...
		var drawRectangleTool = mapClient.map.controls.filter(function(t) {
	...
	mapPanelRender : function(panel) {
		...
		mapClient = Ext.create('OpenEMap.Client'); // global intentionally
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				mapClient.map.addControl(drawRectangelTool);
				
				mapClient.drawLayer.events.register('featureadded', null, function(){
					AdmClient.app.configuration.extent = arguments[0].feature.geometry.getBounds().toArray();
				});
			}
		});
	...
	setConfigurationName : function(combo, e, eOpts){
		AdmClient.app.configuration.name = combo.getValue();
	},
	configurationPreviewPanelBeforeShow : function(panel, eOpts){
		
	},
	configurationPreviewPanelShow : function(panel, eOpts){
		var configTextArea = Ext.ComponentQuery.query('#configurationTextfield')[0];
		configTextArea.setValue(Ext.encode(AdmClient.app.configuration)); 
	},
	
	configurationPreviewPanelHide : function(panel, eOpts){
...
Ext.define('AdmClient.controller.Servers', {
	...
	init : function() {
		this.control({
			'#srvGrid' : {
				edit : this.serversGridEdit
			},
			'#searchGrid' : {
				edit : this.searchGridEdit
			},
			'#addSettingsRow' : {
...
Ext.define('AdmClient.view.mapconfiguration.tools.details.General', {
	...
	constructor : function(){
		this.toolName = arguments[0] ? arguments[0] : '';
		this.callParent(arguments);
	},
	
	addDetail : function(panel){
		this.items.push({
			xtype : 'button'
		});
		this.doLayout();
	...
	initComponent : function() {
		this.items = [ {
			...
			title : 'General settings for tool ' + this.toolName,
...
Ext.define('AdmClient.controller.ToolsGrid', {
	...
	gridRender : function(panel, opts) {
		this.grid = panel;
		this.detailPanel = Ext.ComponentQuery.query('#toolDetail')[0];
	},
	...
	gridRowSelected : function(toolsGrid, record, index, eOpts) {
		...
		this.detailPanel.removeAll(false);
		
		var view = this.views[record.get('tool')] ? this.views[record.get('tool')] : new AdmClient.view.mapconfiguration.tools.details.General(record.get('tool'));

		this.views[record.get('tool')] = view;
		
		this.detailPanel.add(view);
...
Ext.define('AdmClient.store.Servers', {
	...
	data : [ [ 'server1', 'http://server1', 'user1', 'secret1', 'note1' ],
			[ 'server2', 'http://server2', 'user2', 'secret2', 'note2' ] ]
...
Ext.define('AdmClient.store.ToolStore', {
	...
	data : [ [ 'DrawGeometry', 'Draw geometry', 'Action for draw geometry.', false ],
...
Ext.application({
    ...
    controllers: ['Main', 'MainToolbar', 'Servers', 'MapConfiguration', 'ToolsGrid'],
    launch: function() {
    	this.configuration = {};
